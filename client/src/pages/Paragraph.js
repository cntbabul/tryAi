...
const Paragraph = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const { data } = await axios.post("/api/v1/try-ai/paragraph", { text });
        setPara(data.para);
    } catch (err) {
      console.log(error);
  ...
  return (
    ...
    >
      ...
      <form onSubmit={handleSubmit}>
        <Typography variant="h3" textAlign={"center"}>Paragraph</Typography>
...
{para}
      ...
      <Typography variant='h5' color='natural.main' sx={{textAlign:'center', verticalAlign:'middle', lineHeight:'500px'}}>Generated Paragraph will be shown here
